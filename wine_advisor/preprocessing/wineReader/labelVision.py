...
class labelVision:
    ...
    #     src_ocr = pytesseract.image_to_string(src_gray, config=self.Config['ocr_mode'])
    #     unwrapped_ocr = pytesseract.image_to_string(unwrapped_gray, config=self.Config['ocr_mode'])
    ...
    def readLabels(self, unet_output, srcs, fileNames):
        ...
        for mask, src, filename in zip(unet_output, srcs, fileNames):
            ...
            save_img(os.environ['UNET_RESULTS_PATH'] + filename + "/3_resize_binary_mask.jpg", mask)

            # rotate
            r_src, r_mask = self.align_vertically(src, mask)
            save_img(os.environ['UNET_RESULTS_PATH'] + filename + "/4_rotate_mask.jpg", r_mask)
            save_img(os.environ['UNET_RESULTS_PATH'] + filename + "/5_rotate_src.jpg", r_src)

            # search cylindric edges points in label structure
            r_mask = np.uint8(r_mask*255)
            shape, img_point = self.getCylinderPoints(r_mask)
            save_img(os.environ['UNET_RESULTS_PATH'] + filename + "/6_points.jpg", img_point)

            # unwrap label
            mesh, unwrapped = self.unwrapLabel(r_src, shape)
            cv2.imwrite(os.environ['UNET_RESULTS_PATH'] + filename + "/7_mesh.jpg", mesh)
            cv2.imwrite(os.environ['UNET_RESULTS_PATH'] + filename + "/8_unwrapped.jpg", unwrapped)

            # Add the 9th grayscale image (e.g., grayscale version of the unwrapped image)
            unwrapped_gray = cv2.cvtColor(unwrapped, cv2.COLOR_BGR2GRAY)
            cv2.imwrite(os.environ['UNET_RESULTS_PATH'] + filename + "/9_unwrapped_gray.jpg", unwrapped_gray)

            # tesseract ocr
            # unwrapped_ocr, src_ocr = self.ocr(src, unwrapped)
            # to_write_unw_ocr=open(os.environ['UNET_RESULTS_PATH'] + filename + "/10_unwrapped_ocr.txt",'w')
            # to_write_unw_ocr.write(unwrapped_ocr)
            # to_write_unw_ocr.close()
            # to_write_src_ocr=open(os.environ['UNET_RESULTS_PATH'] + filename + "/11_source_ocr.txt",'w')
            # to_write_src_ocr.write(src_ocr)
            # to_write_src_ocr.close()

            images.append((unwrapped_gray, unwrapped))
